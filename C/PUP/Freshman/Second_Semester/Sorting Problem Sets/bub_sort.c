...
{
    int a, b, duplicate;
    printf("Input five numerical values for array %c\n\n", letter);
    ...
    for (a = 0; a < n; a++)
    {
        do
        {
            ...
            {
                printf("[Note: Non-numerical values are not allowed!]\n\n");
                printf("%c[%d]: ", letter, a + 1);                
                while (getchar() != '\n');
            }
        ...
        while (duplicate == 1); // Repeat the loop if there is a detected duplicate           
    }
    printf("\n");
